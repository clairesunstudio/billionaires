function graphBillionaires(){

var bardata = [];
var country = [];

for(key in billionaires) {
    bardata.push(billionaires[key].radius)
    country.push(billionaires[key].country)
}
    ...
    // .range(['#B58929','#268BD2', '#268BD2', '#85992C'])
    // .range(['#85992C','#268BD2', '#268BD2', '#B58929'])
    .range(['#F9A74B','#FFFF66'])
    ...
    var xScale = d3.scale.ordinal()
            ...
            .rangeBands([0, bardata.length*60], 0.2)
    ...
    var vAxis = d3.svg.axis()
        ...
        .orient('left')
        .ticks(10)
    var hGuide = d3.select('#timeline').select('svg').append('g')
        hAxis(hGuide)
        hGuide.attr('transform', 'translate(' + margin.left + ', ' + (height + margin.top) + ')')
        hGuide.selectAll('path')
            .style({ fill: 'none', stroke: "white"})
